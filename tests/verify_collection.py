...
def fetch_bearer_token(env: str, request_session: requests.Session) -> str:
...
def fetch_bearer_token_by_provider(env: str, request_session: requests.Session, token_provider: str) -> str:
    if token_provider == "lambda":
        ...
        lambda_function_name = f"{env}-launchpad_token_dispenser"
        ...
        return get_bearer_token_via_lambda(
            ...
            cmr_env=env,
            client_id=os.environ.get("CMR_TOKEN_CLIENT_ID", "l2ss-py-autotest-github-action"),
            aws_region=os.environ.get("CMR_TOKEN_LAMBDA_REGION", "us-west-2"),
        )

    return fetch_bearer_token(env, request_session)
...
def get_bearer_token_via_lambda(lambda_function_name, edl_user, edl_pass, cmr_env, client_id="l2ss-py-autotest-github-action", aws_region="us-west-2"):
    """
    Calls the specified Lambda function to get a bearer token for EDL.
    """
    import boto3
